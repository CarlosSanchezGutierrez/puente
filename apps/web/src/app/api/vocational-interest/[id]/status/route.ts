  ...
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { ok: false, error: "Falta el identificador del registro." },
        { status: 400 },
      );
    }
    ...
    if (error) {
      console.error("vocational status update error:", error);
    ...
    console.error("vocational status PATCH exception:", error);
